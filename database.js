...
(function(){
  ...
  Store.open().then(function(res){
    ...
    // Split deployments meet ONLY in Supabase. Without it this app shows
    // its own empty storage while the scanner fills a different one.
    if(PDZ.isSplit() && store.mode !== "cloud"){
      say("<b>This list cannot see the scanner.</b> The two apps are deployed " +
          "separately, so they share data only through Supabase — fill in " +
          "supabaseUrl and supabaseAnonKey in config.js.", true);
    } else {
      say(res.fellBack
